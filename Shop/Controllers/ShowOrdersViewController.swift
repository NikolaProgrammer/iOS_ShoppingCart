...
extension ShowOrdersViewController: UITableViewDataSource {
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        return queryOrderService.orders.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let order = queryOrderService.orders[indexPath.section]
...
extension ShowOrdersViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            let order = queryOrderService.orders.remove(at: indexPath.section)
